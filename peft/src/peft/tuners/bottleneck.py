...
class BottleneckModel(torch.nn.Module):
    ...
    **Attribute**:
    ...
    def _process_adapter_hook(self, key):
    ...
    def _find_and_replace(self):
        ...
        for key in key_list:
            if isinstance(self.peft_config.target_modules, str):#先找Target 
                target_module_found = re.fullmatch(self.peft_config.target_modules, key)
            else:
                target_module_found = any(key.endswith(target_key) for target_key in self.peft_config.target_modules)
            ##############add lora hook
            if target_module_found and self.prune_module:
                target_module_found = self._process_adapter_hook(key)
            ##############
...
class Linear(nn.Linear, AdapterLayer):
    ...
    def cal_imp(self,adapter_input,adapter_output):
        if self.peft_imp and self.adapter_up.training:
            if "p2" in self.peft_imp:
                # output vector use L2Norm
                self.pos_importance_scores.append(
                    torch.div(torch.norm(adapter_output, p=2, dim=2),torch.norm(adapter_input, p=2, dim=2)).detach().cpu()
                )
